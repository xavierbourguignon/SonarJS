...
function identities(x) {
  let y = x;
  let z = x;
  z = y; // Noncompliant {{Remove this useless assignment; "z" already holds the assigned value along all execution paths.}}
//^^^^^
}

function strictConstraints(x, y) {
  if (x === "" && y === "") {
    x = y; // Noncompliant {{Remove this useless assignment; "x" already holds the assigned value along all execution paths.}}
//  ^^^^^
  }
}
...
function nonStrictConstraints(x, y) {
  if (x === "hello" && y === "hello") {
    x = y; // OK
  }
}
...
function exceptions() {
  ...
  x = y.foo(); // OK we don't evaluate properties

  let array = [0];
  let e = 0;
  e = array[0]; // OK we ignore subscript values
