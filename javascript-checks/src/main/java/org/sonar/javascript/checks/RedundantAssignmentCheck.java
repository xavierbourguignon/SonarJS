...
public class RedundantAssignmentCheck extends AbstractAllPathSeCheck<AssignmentExpressionTree> {
  ...
  private static final String MESSAGE = "Remove this useless assignment; \"%s\" already holds the assigned value along all execution paths.";
  ...
  boolean isProblem(AssignmentExpressionTree tree, ProgramState currentState) {
    ...
    if (tree.variable().is(Tree.Kind.IDENTIFIER_REFERENCE) && tree.expression().is(Tree.Kind.IDENTIFIER_REFERENCE)) {
      if (((IdentifierTree) tree.variable()).name().equals(((IdentifierTree) tree.expression()).name())) {
        return false;
      }
    }
    ...
    if (leftConstraint.isSingleValue() && rightConstraint.isSingleValue() && leftConstraint.equals(rightConstraint)) {
      return true;
    }
    return false;
  }
